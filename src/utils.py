...
class Portfolio:
    ...
    def __init__(self, stocks, weights, data) :
        self.stocks = stocks #list of stocks
        self.weights = weights #list of associated weights
        self.data = data #dataframe containing for each stock adj close
        #self.initial_value = initial_value # initial investment
        
    def get_norm_return(self):
       return self.data[self.stocks].pct_change(1)
    
    def get_cum_return(self):
       return self.data[self.stocks]/self.data[self.stocks].iloc[0]
...
def plot_correlation(df):# Evaluate the matrix correlation
    corr = df.corr()
